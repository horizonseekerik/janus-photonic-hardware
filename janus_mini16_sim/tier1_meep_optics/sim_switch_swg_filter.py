...
def make_swg_barrier_blocks(x_start, x_end, y_center=0.0, h_swg=0.30, period=0.20, duty_cycle=0.50):
    """
    Creates periodic Sub-Wavelength Grating (SWG) silicon strips in the inter-arm cladding.
    Period Lambda = 200 nm << lambda / n (true deep sub-wavelength regime).
    Duty cycle = 50% (100 nm Si, 100 nm SiO2).
    Acts as an anisotropic metamaterial barrier that scatters/blocks transverse radiation (ky != 0)
    while maintaining total internal reflection for on-axis guided modes (ky = 0).
    """
    w_strip = period * duty_cycle
    n_periods = int(math.floor((x_end - x_start) / period))
    blocks = []
    
    mat_core = mp.Medium(index=N_CORE)
    for i in range(n_periods):
        x_c = x_start + (i + 0.5) * period
        blocks.append(
            mp.Block(
                size=mp.Vector3(w_strip, h_swg, mp.inf),
                center=mp.Vector3(x_c, y_center, 0),
                material=mat_core
            )
        )
    return blocks

def simulate_switch_with_swg_filter(h_swg=0.30, l_filter=2.00):
    """
    Simulates the 1x2 switch integrated with Method 1:
    Sub-Wavelength Grating (SWG) Anisotropic Cladding Filter between the output ports.
    """
    print("=" * 80)
    print("SIMULATING SWITCH WITH SUB-WAVELENGTH GRATING (SWG) ANISOTROPIC FILTER (METHOD 1)")
    print("=" * 80)
    ...
    for state in ["amorphous", "crystalline"]:
        ...
        # Output straight waveguides through filter region
        x_f_s = x_sb_e
        ...
        # METHOD 1: Sub-Wavelength Grating (SWG) Anisotropic Filter in the inter-port cladding
        # Spans y in [-h_swg/2, +h_swg/2]. With h_swg = 0.30 um, y in [-0.15, +0.15].
        # The waveguide edges are at y = +/-0.26 um -> 110 nm buffer!
        swg_blocks = make_swg_barrier_blocks(x_f_s, x_f_e, y_center=0.0, h_swg=h_swg, period=0.20, duty_cycle=0.50)
        geom.extend(swg_blocks)
    ...
    print("\n" + "=" * 80)
    print("FINAL CONVERGED RESULTS FOR METHOD 1 (SWG ANISOTROPIC CLADDING FILTER):")
...
if __name__ == "__main__":
    simulate_switch_with_swg_filter(h_swg=0.30, l_filter=2.00)
