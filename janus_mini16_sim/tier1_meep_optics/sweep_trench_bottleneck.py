...
def sweep_trench_bottleneck():
    """
    Sweeps the bottleneck width W_neck with the Deep Isolation Air Trench to find
    the exact sweet spot where:
      - Amorphous Transmission > 95% (IL < 0.22 dB)
      - Crystalline Crosstalk <= -30 dB (Leakage <= 0.10%)
    """
    w_wg = 0.28
    gap = 0.080
    l_c = 3.80
    ...
    for w_neck in [0.23, 0.21, 0.20]:
        ...
        print(f"W_neck={w_neck*1000:.0f}nm | AMORPH: Cross={p_am_cross*100:5.2f}% (IL={il_am:.3f}dB), XT={xt_am:6.2f}dB | CRYST: Bar={p_cr_bar*100:5.2f}% (IL={il_cr:.3f}dB), XT={xt_cr:6.2f}dB", flush=True)

if __name__ == "__main__":
    sweep_trench_bottleneck()
