...
Fig.~\ref{fig:spice_ber_waterfall} plots the receiver Bit Error Rate (BER) waterfall curve as a function of received optical carrier power $P_{\mathrm{rx}}$, comparing the analytical $Q$-factor formulation against empirical 1,000,000-bit SPICE verification checkpoints. The nominal delivered power ($P_{\mathrm{rx}} = -14.63\,\mathrm{dBm}$) delivers an analytical $\mathrm{BER} = 1.097 \times 10^{-58}$, maintaining a $+8.41\,\mathrm{dB}$ operating headroom above the $-25.05\,\mathrm{dBm}$ receiver sensitivity limit. Furthermore, Fig.~\ref{fig:strongarm_regen_hist} presents the empirical regeneration delay distribution of the clocked 65nm CMOS StrongARM comparator over 1,000,000 cycles. Because all latch decisions resolve within $3.8 - 4.9\,\mathrm{ps}$ (mean $4.18\,\mathrm{ps}$), latching completes strictly within the $5.0\,\mathrm{ps}$ evaluation half-cycle, eliminating metastability events across the entire 1M workload.
...
\caption{Transient regeneration latency distribution for the clocked 65nm CMOS StrongARM latch across 1,000,000 decision cycles. With a regeneration time constant $\tau_{\mathrm{regen}} = 0.65\,\mathrm{ps}$, all latching events resolve strictly within $3.8 - 4.9\,\mathrm{ps}$, well below the 8.0~ps metastability warning floor and the 10.0~ps clock cycle boundary.}
\label{fig:strongarm_regen_hist}
\end{figure}
